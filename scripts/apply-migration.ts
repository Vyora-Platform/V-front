...
if (!databaseUrl) {
  console.error("ERROR: DATABASE_URL or NEW_DATABASE_URL environment variable is required");
  process.exit(1);
}
...
async function applyMigration() {
  ...
  try {
    ...
    const result = await client.query(`
      ...
      WHERE table_name = 'vendor_catalogues' 
      AND column_name = 'duration_type';
    `);
    
    if (result.rows.length > 0) {
      console.log("✅ Verified: duration_type column exists");
    } else {
      console.log("⚠️ Warning: duration_type column verification failed");
    }
