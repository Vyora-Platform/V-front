...
export default function VendorLedgerSupplierTransaction() {
  const params = useParams<{ vendorId: string; supplierId: string }>();
  ...
  const transactionType = (searchParams.get("type") as "in" | "out") || "out";
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      category: transactionType === "in" ? "refund" : "purchase",
      paymentMethod: "cash",
      description: "",
      note: "",
      isRecurring: false,
  ...
  const createMutation = useMutation({
    mutationFn: async (data: FormData) => {
      const response = await apiRequest("POST", `/api/vendors/${vendorId}/ledger-transactions`, {
        ...data,
        vendorId,
      });
      return response.json();
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/suppliers", supplierId, "ledger-transactions"] });
      navigate(`/vendors/${vendorId}/ledger/supplier/${supplierId}`);
    },
    onError: (error: any) => {
      toast({
        title: "Error",
        ...
        <div className="flex items-center justify-between px-4 py-3 text-white">
          <div className="flex items-center gap-3">
            <Button
              variant="ghost"
              size="icon"
              onClick={() => navigate(`/vendors/${vendorId}/ledger/supplier/${supplierId}`)}
            ...
            <div>
              <h1 className="text-lg font-semibold">
                {transactionType === "in" ? "You Got Money" : "You Paid Money"}
              </h1>
              <p className="text-xs opacity-90">
                {transactionType === "in" ? "Received from" : "Paid to"} {supplier?.name}
              </p>
            </div>
          </div>
        </div>
